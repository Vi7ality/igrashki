...
export const fetchToysAdmin = createAsyncThunk(
  'toysAdmin/fetchToys',
  async () => {
    try {
...
export const addToyAdmin = createAsyncThunk(
  'toysAdmin/addToy',
  async (toy: Partial<IToy>) => {
    try {
      const managerToken = localStorage.getItem('managerToken');
...
export const updateToyAdmin = createAsyncThunk(
  'toysAdmin/updateToy',
  async (toy: Partial<IToy>) => {
    try {
      const managerToken = localStorage.getItem('managerToken');
...
export const deleteToyAdmin = createAsyncThunk(
  'toysAdmin/deleteToy',
  async (toyId: string) => {
    try {
      const managerToken = localStorage.getItem('managerToken');
      ...
      toast.success('Іграшка видалена з точки');
      return toyId;
    } catch (error) {
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(updateToyAdmin.fulfilled, (state, action) => {
        const toyIndex = state.toysAdmin.findIndex(
          toy => toy._id === action.payload?._id
        );
        if (toyIndex !== -1) {
          state.toysAdmin[toyIndex] = {
            ...action.payload,
            toyName: state.toysAdmin[toyIndex].toyName,
          };
        }
      })
      .addCase(deleteToyAdmin.fulfilled, (state, action) => {
        state.toysAdmin = state.toysAdmin.filter(
          toy => toy._id !== action.payload
        );
      });
  },
});
