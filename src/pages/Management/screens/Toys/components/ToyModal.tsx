...
import api from '../../../../../api';
import { addToyAdmin, updateToyAdmin } from '../../../../../redux/slices/toysAdmin.slice';
...
const getValueBySelectedOption = (option: { label: string; value: string } | null | undefined) => {
    return option?.value ? option.value : (option ? option : "") as string
}

const transformEditableToy = (editableToy: IToy): IToyForm => {
    const toyState = toyStateOptions.find((option) => option.value === editableToy.toyState)
    const toyId = { label: editableToy.toyName, value: editableToy.toyId }
    return { ...editableToy, toyState, toyId, lastDisinfectionDate: format(new Date(editableToy.lastDisinfectionDate), 'yyyy-MM-dd') }
}

const ToyModal: FC<ToyModalProps> = ({ isModalOpen, closeModal, editableToy }) => {
    const dispatch = useAppDispatch()
    const { register, control, handleSubmit, reset } = useForm<IToyForm>(
        { mode: 'onBlur', values: editableToy ? transformEditableToy(editableToy) : defaultValues }
    );
    ...
    useEffect(() => {
        const fetchToys = async () => {
            ...
            setToysFullList(data.map((toy: IToy) => ({ label: toy.toyName, value: toy.toyId })))
        }
        fetchToys()
    }, [])
            ...
            <div className={styles.modalContent}>
                ...
                <form className={styles.toyForm} onSubmit={handleSubmit(onSubmit)}>
                    {editableToy ?
                        ...
                        : toysFullList?.length > 0 && (<Controller control={control} name='toyId' render={({ field }) => (
                            <Select label='Іграшка' {...field} options={toysFullList} />
                        )} />)}

                    {!editableToy && <Input label="Кількість іграшок" {...register('quantity')} name='quantity' />}
                    <Input label='Цикл користування' {...register('usageCycle')} name='usageCycle' />
                    <Input label='Дата дезінфекції' type='date' {...register('lastDisinfectionDate')}  name='lastDisinfectionDate' />
                    <Controller control={control} name='toyState' render={({ field }) => (
                        <Select label='Стан іграшки' {...field} options={toyStateOptions} />
                    )} />
                    <button type="submit">
                        Submit
                    </button>
