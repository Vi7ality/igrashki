...
const AddToyModal: FC<AddToyModalProps> = ({ isModalOpen, closeModal, editableToy }) => {
    const [images, setImages] = useState<string[]>(editableToy ? editableToy.images : [])
    const [image, setImage] = useState<string>('')
    const [features, setFeatures] = useState<string[]>(editableToy ? editableToy.features : [])
    const [feature, setFeature] = useState<string>('')
    const [isAddToyOpen, setIsAddToyOpen] = useState(false)
    const [isAddFeatureOpen, setIsAddFeatureOpen] = useState(false)
    const { register, handleSubmit } = useForm<IToyForm>(
        { mode: 'onBlur', values: editableToy ? editableToy : defaultValues }
    );
